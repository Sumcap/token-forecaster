@echo off
rem Windows entry point for the draft-aware Claude Code launcher.
rem
rem `bin/tf-claude` is a Python program with a `#!` line, which Windows does not
...
rem
rem Every failure here ends the same way -- by starting Claude Code unchanged.
rem Losing the forecast is a bad afternoon; losing `claude` is a broken machine.
setlocal
set "TF_SCRIPT=%~dp0tf-claude"
set "TF_PY="
...
:nopython
>&2 echo tf-claude: no Python 3 on PATH, so this session has no draft forecast.
