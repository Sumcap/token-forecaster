...
pseudo-user (PolymarketDashboard 257 session files, OutcomeHLBot 121,
sheep-manager 92, token-forecaster 74, MMingResearchingExecution 43,
petbot-monorepo 21, Projects 19, MMingPolymarket 15). That gives 6 to 8
...
- **Path (a), opt-in telemetry to the ingest VM.** The client shipped in
  sheep-manager, and `userIdHash` and `workloadIdHash` are already in the
  ...
  weak telemetry source; the exact-usage sources are sheep-manager, CLI users
